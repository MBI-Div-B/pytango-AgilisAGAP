...
class AgilisAGAP(Device):
    ...
    def init_device(self):
        ...
        try:
            self.info_stream(
                "Connecting to AgilisAGAP on port: {:s} ...".format(self.Port)
            )
    ...
    def write_position_x(self, value):
        ...
        if err in self.__ERROR_OUT_OF_RANGE:
            Except.throw_exception(
                "x position out of range", "x position out of range", "write_position_x"
            )
        else:
            self.set_state(DevState.MOVING)
    ...
    def write_position_y(self, value):
        ...
        if err in self.__ERROR_OUT_OF_RANGE:
            Except.throw_exception(
                "y position out of range", "y position out of range", "write_position_y"
            )
        else:
            self.set_state(DevState.MOVING)
